...
class NoisePruningExperiment:
    ...
    def prepare_data(self) -> Dict:
        ...
        try:
            ...
            print(
                f"   噪声样本: {len(noise_indices)} ({len(noise_indices) / len(noisy_data['y_train']) * 100:.1f}%)"
            )
    ...
    def compute_influence_scores(self, model, data: Dict) -> np.ndarray:
        ...
        try:
            ...
            analysis = self.tim_calculator.analyze_influence_scores(
                influence_scores, data["y_train"], noise_indices
            )

            # 保存影响力结果
            self.tim_calculator.save_influence_results(
                influence_scores, analysis, str(self.output_dir / "influence_analysis")
            )
    ...
    def prune_data(self, influence_scores: np.ndarray, original_data: Dict) -> Dict:
        ...
        try:
            ...
            self.data_processor.save_data(
                pruned_data, str(self.output_dir / "pruned_data")
            )
            ...
            print("✅ 数据剪枝完成")
            print(
                f"   剪枝样本: {len(prune_indices)} ({len(prune_indices) / len(original_data['y_train']) * 100:.1f}%)"
            )
            print(
                f"   噪声捕获: {len(pruned_noise)}/{len(noise_indices)} (召回率: {noise_recall:.2%})"
            )
    ...
    def train_pruned_model(self, pruned_data: Dict, initial_state: Dict) -> Dict:
        ...
        try:
            ...
            # 保存训练历史
            self.bert_trainer.save_training_history(
                pruned_history, str(self.output_dir / "pruned_training")
            )
    ...
    def run_complete_experiment(self) -> Dict:
        ...
        try:
            ...
            # 4. 计算影响力分数
            influence_scores, influence_analysis = self.compute_influence_scores(
                original_model, noisy_data
            )

            # 5. 剪枝数据
            pruned_data, prune_indices, keep_indices = self.prune_data(
                influence_scores, noisy_data
            )

            # 6. 训练剪枝后模型
            pruned_model, pruned_history = self.train_pruned_model(
                pruned_data, initial_state
            )
    ...
    def generate_comparative_analysis(
        self, original_history: Dict, pruned_history: Dict
    ):
        """生成对比分析结果"""
        print("📈 生成对比分析...")
            ...
            comparative_analysis = {
                "performance_improvement": {
                    "train_accuracy": {
                        ...
                        "improvement": pruned_final.get("train_accuracy", 0)
                        - orig_final.get("train_accuracy", 0),
                    },
                    "valid_accuracy": {
                        "original": orig_final.get("valid_accuracy", 0),
                        "pruned": pruned_final.get("valid_accuracy", 0),
                        "improvement": pruned_final.get("valid_accuracy", 0)
                        - orig_final.get("valid_accuracy", 0),
                    },
                    "train_loss": {
                        "original": orig_final.get("train_loss", 0),
                        "pruned": pruned_final.get("train_loss", 0),
                        "improvement": orig_final.get("train_loss", 0)
                        - pruned_final.get("train_loss", 0),  # 损失越低越好
                    },
                },
                "training_efficiency": {
                    "original_time": orig_final.get("total_time", 0),
                    "pruned_time": pruned_final.get("total_time", 0),
                    "time_saving": orig_final.get("total_time", 0)
    ...
    def save_experiment_results(self):
        ...
        try:
            # 保存主结果文件
            with open(
                self.output_dir / "experiment_results.json", "w", encoding="utf-8"
            ) as f:
    ...
    def print_experiment_summary(self):
        ...
        print(
            f"⏱️  总耗时: {self.results.get('end_time', 0) - self.results.get('start_time', 0):.1f}秒"
        )
        ...
        if data_stats:
            ...
            print(
                f"   噪声样本: {noise_info.get('noise_count', 'N/A')} ({noise_info.get('noise_rate', 0) * 100:.1f}%)"
            )
        ...
        if comparative:
            ...
            print(
                f"   训练准确率: {train_acc.get('original', 0):.3f} → {train_acc.get('pruned', 0):.3f} ({train_acc.get('improvement', 0):+.3f})"
            )

            valid_acc = perf.get("valid_accuracy", {})
            print(
                f"   验证准确率: {valid_acc.get('original', 0):.3f} → {valid_acc.get('pruned', 0):.3f} ({valid_acc.get('improvement', 0):+.3f})"
            )

            train_loss = perf.get("train_loss", {})
            print(
                f"   训练损失: {train_loss.get('original', 0):.3f} → {train_loss.get('pruned', 0):.3f} ({train_loss.get('improvement', 0):+.3f})"
            )
