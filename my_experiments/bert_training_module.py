...
class BertTrainer:
    ...
    def train_model(
            ...
            for batch_start in range(0, len(x_train), batch_size):
                ...
                try:
                    ...
                    if step % eval_steps == 0:
                        valid_loss, valid_acc = self.evaluate_model(model, x_valid, y_valid)
                        history['valid_loss'].append(valid_loss)
                        history['valid_accuracy'].append(valid_acc)
                        print(f"   🔍 验证 - Loss: {valid_loss:.4f}, Accuracy: {valid_acc:.4f}")
                        model.train()  # 回到训练模式
        ...
        print(f"📊 最终性能:")
        print(f"   训练集 - Loss: {final_train_loss:.4f}, Accuracy: {final_train_acc:.4f}")
        print(f"   验证集 - Loss: {final_valid_loss:.4f}, Accuracy: {final_valid_acc:.4f}")
    ...
    def save_training_history(self, history: Dict, save_path: str):
        ...
        np.savez(
            ...
            **{k: np.array(v) if isinstance(v, list) else v for k, v in history.items() if k != 'final_performance'}
        )
