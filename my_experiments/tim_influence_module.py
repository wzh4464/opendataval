...
class BertTimInfluenceCalculator:
    ...
    def setup_tim_evaluator(self) -> TimInfluence:
        ...
        self.tim_evaluator = TimInfluence(
            start_step=self.t1,
            end_step=self.t2,
            time_window_type='full' if self.t1 == 0 and self.t2 is None else 'custom_range',
    ...
    def compute_influence(
        ...
        valid_attention_mask = valid_dataset.tensors[1].to(device)
    ...
    def analyze_influence_scores(
        ...
        if noise_indices is not None:
            ...
            stats['noise_analysis']['mean_noise_rank_percentile'] = float(np.mean(noise_ranks))
    ...
    def select_bottom_k_samples(
        ...
        print(f"   剪枝样本影响力范围: [{influence_scores[prune_indices].min():.6f}, "
