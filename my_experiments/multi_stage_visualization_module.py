...
class MultiStageVisualizer:
    ...
    def plot_comprehensive_dashboard(
        ...
        # 7. 统计摘要已在上方通过 _plot_experiment_summary 生成
...
• Time-aware data cleaning strategies show promise for improving model robustness"""
    ...
    def _plot_experiment_summary(
        ...
        summary_text = (
            ...
            f"• Dataset: {experiment_config.get('dataset_name', 'N/A').upper()}\n"
            f"• Total Training Samples: {total_samples:,}\n"
            f"• Injected Noise Samples: {noise_count:,}\n"
            f"• Noise Rate: {noise_rate*100:.1f}%\n"
            f"• Time Window Stages: {experiment_config.get('num_stages', 5)}\n\n"
            f"Performance Highlights:\n"
            f"• Best Training Accuracy: {best_train_acc:.3f}\n"
            f"• Best Validation Accuracy: {best_valid_acc:.3f}\n"
            f"• Best Noise Recall: {best_noise_recall:.1%}\n"
            f"• Best Noise Precision: {best_noise_precision:.1%}\n\n"
            f"Model Configuration:\n"
            f"• Architecture: {experiment_config.get('model_name', 'N/A')}\n"
        )
        ax.text(
            0.05,
            0.95,
