...
def run_stage_lava(
    ...
    with torch.no_grad():
        x_train_emb = emb_model.predict(x_train).detach().to(device)
        x_valid_emb = emb_model.predict(x_valid).detach().to(device)
    ...
    lava.input_data(x_train_emb, y_train_dev, x_valid_emb, y_valid_dev)
